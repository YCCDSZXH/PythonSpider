...
def get_data(page, startage, endage, gender, startheight, endheight, salary):
    ...
    # 网站链接
    # http://www.lovewzly.com/api/user/pc/list/search?startage=21&endage=30&gender=2&marry=1&salary=2&page=1
    # http://www.lovewzly.com/api/user/pc/list/search?startage=21&endage=30&gender=2&cityid=197&marry=1&salary=2&page=1

    # 老师你代码我能看懂，但是我自己写不出来
    # 其实学习是讲究方法的 接下来大家想不想听一下老师给大家的学习方法
    # 今天先跟大家说一个,就是你要有一个清晰明确的学习路线图
    # 如果大家愿意按照老师的这个学习线路图来学习  愿意每天为自己投资20块钱左右的同学 去加QQ

    base_url = 'http://www.lovewzly.com/api/user/pc/list/search?' + 'page={}&startage={}&endage={}&gender={}&cityid=52&startheight={}&endheight={}&marry=1&educatin=40&salary={}'.format(
        page, startage, endage, gender, startheight, endheight, salary)
    # 拼接请求参数
...
def query_data():
    ...
    for i in range(1, 10):
        # 获取抓取到的 json 数据
        base_url = 'http://www.lovewzly.com/api/user/pc/list/search?' + 'page={}&startage={}&endage={}&gender={}&cityid=52&startheight={}&endheight={}&marry=1&educatin=40&salary={}'.format(
            i, startage, endage, gender, startheight, endheight, salary)
        # 拼接请求参数
        # url = base_url + urlencode(params)
        ...
        if response.status_code == 200:
            # 返回 json 数据
            # return response.json()
            wzly_json = response.json()
            for item in wzly_json['data']['list']:
                save_image(item)
                save_to_txt(item)


        # json = get_data(i, startage, endage, gender, startheight, endheight, salary)
        # # print(json['MNIST_data']['list'])
        # # 循环遍历每个 json 数据
        # for item in json['MNIST_data']['list']:
        #     # 保存图片
        #     save_image(item)
        #     save_to_txt(item)
...
def save_image(item):
    ...
    if not os.path.exists('images'):
        # 创建 images 文件夹     是一级一级的创建目录
        # os.makedirs(path)     他可以一次创建多级目录
        os.mkdir('images')
    try:
        # 获得 image 信息
...
def save_to_txt(item):
    with open('grilinfo/' + item['username'] + '.txt', 'w', encoding='utf-8') as f:
        f.write('名字:' + item['username'] + ',城市:' + item['city'] + ',身高:' + item['height'] + ',学历:' + item[
            'education'] + ',生日:' + item['birthdayyear'])
